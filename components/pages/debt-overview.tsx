...
import { AnnualIssuanceDataset, AnnualBalanceDataset } from "@/lib/types";
import { SimpleBarChart } from "@/components/charts/simple-bar-chart";
import { SimpleLineChart } from "@/components/charts/simple-line-chart";
import { MultiLineChart } from "@/components/charts/multi-line-chart";

export function DebtOverview({ items, annualIssuance, annualBalance }: { items: DebtDataItem[]; annualIssuance: AnnualIssuanceDataset; annualBalance: AnnualBalanceDataset }) {
  ...
  const balanceTotalSeries = annualBalance.series.find((s) => s.key === "total");
  const balanceGeneralSeries = annualBalance.series.find((s) => s.key === "general");
  const balanceSpecialSeries = annualBalance.series.find((s) => s.key === "special");

  const balanceChartSeries = [
    balanceTotalSeries && {
      label: balanceTotalSeries.label,
      color: "#1B4965",
      values: balanceTotalSeries.values.map((v) => ({ label: `${v.year}`, value: v.value })),
    },
    balanceGeneralSeries && {
      label: balanceGeneralSeries.label,
      color: "#5FA8D3",
      values: balanceGeneralSeries.values.map((v) => ({ label: `${v.year}`, value: v.value })),
    },
    balanceSpecialSeries && {
      label: balanceSpecialSeries.label,
      color: "#8B0000",
      values: balanceSpecialSeries.values.map((v) => ({ label: `${v.year}`, value: v.value })),
    },
  ].filter(Boolean) as { label: string; color: string; values: { label: string; value: number }[] }[];
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <article className="info-card p-4">
          ...
          <MultiLineChart series={balanceChartSeries} unit="亿元" />
        </article>
        <article className="info-card p-4">
