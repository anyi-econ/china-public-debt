...
description: "Find Chinese province / city / county government 本地要闻 / 政务动态 / 政府要闻 entry pages by drilling down from government portals (政府门户) listed in data/website-gov.ts. USE WHEN: user asks to find regional local-news index URLs, supplement data/website-news.ts or data/website-news.xlsx, populate 地区新闻导航, or mentions 本地要闻/政务动态/政府要闻/今日要闻 in this project. ASSUMES gov portals in data/website-gov.ts are already validated — does NOT re-verify them."
---
...
> **前提**：`data/website-gov.ts` 里的政府门户 URL 默认已通过校验，本 skill **不再**做政府官网可达性 / 真伪校验，直接当作可信入口下钻。如发现某条 gov URL 明显失效，**只标记 `unreachable` 让人工另行修复**，不在本流水线里阻断。
...
Step 2: 在首页 HTML 里定位"新闻 / 要闻 / 政务动态"类栏目候选
Step 3: 按"本地综合 > 部门动态 > 上级要闻"优先级择优 + 验证候选页
Step 4: 写入 website-news.xlsx 并记录 log
```

> 政府门户的可达性 / 真伪校验**不在本 skill 范围内**，由 `gov-site-finder` 单独负责并已落到 `data/website-gov.ts`。本 skill 只信任输入。
...
1. fetch_webpage 政府门户首页（**不做真伪校验**，仅作为下钻入口）；DNS / 超时 → 标记 unreachable 后跳过。
