...
// Extended path patterns for Guizhou sites
const paths = [
...
// Only counties that were NO MATCH in probe3
const counties = [
...
function check(url) {
  return new Promise(resolve => {
    const timer = setTimeout(() => resolve({ code: -2 }), 4000);
    try {
      ...
      const req = mod.get(url, { 
        ...
        res.resume();
        res.on('end', () => { clearTimeout(timer); resolve({ code: res.statusCode }); });
        res.on('error', () => { clearTimeout(timer); resolve({ code: -1 }); });
      });
      req.on('error', () => { clearTimeout(timer); resolve({ code: -1 }); });
      req.on('timeout', () => { req.destroy(); clearTimeout(timer); resolve({ code: -2 }); });
    } catch(e) { clearTimeout(timer); resolve({ code: -3 }); }
  });
}
...
async function main() {
  ...
  for (let ci = 0; ci < counties.length; ci++) {
    ...
    for (let pi = 0; pi < paths.length; pi++) {
      if (httpsResults[pi].code === 200) {
        results[county.n].push(`https://${county.d}${paths[pi]}`);
      }
    }
    
    // If no HTTPS hits, try HTTP
    if (results[county.n].length === 0) {
      const httpResults = await Promise.all(
        paths.map(p => check(`http://${county.d}${p}`))
      );
      for (let pi = 0; pi < paths.length; pi++) {
        if (httpResults[pi].code === 200) {
          results[county.n].push(`http://${county.d}${paths[pi]}`);
        }
      }
    }
    
    const status = results[county.n].length > 0 ? 'OK' : '--';
  ...
  for (const county of counties) {
    ...
    if (r.length > 0) {
      lines.push(`OK | ${county.c} | ${county.n} | ${r.join(' ; ')}`);
      found++;
    } else {
  ...
  fs.writeFileSync('scripts/_guizhou_probe4_results.txt', output, 'utf-8');
  console.log(output);
}
